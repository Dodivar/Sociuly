// Accès Prisma au détail d'une expérience publiée — module SERVEUR uniquement.
//
// Séparé de `lib/marketplace/experience-detail.ts` (pur : types + formatage)
// pour qu'un Client Component puisse importer un type ou un formateur sans
// embarquer le driver `pg` (Node-only) dans le bundle navigateur. Garder la
// signature async et les enums alignés sur le schéma Prisma. Tous les montants
// sont en centimes (Int), jamais de float (SPEC §3).
//
