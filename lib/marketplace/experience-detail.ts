// Détail d'une expérience publiée — types et helpers PURS pour la page
// /experiences/[slug]. Cf. SPEC.md §3 (Experience / ExperienceSegment) et §6
// (page détail + CTA « Demander un devis »).
//
// Ce module reste pur (types + formatage) pour qu'un Client Component puisse
// l'importer sans embarquer le driver `pg` (Node-only). L'accès Prisma
// (getExperienceBySlug, getAllExperienceSlugs) vit dans
// `lib/marketplace/experience-detail.server.ts`. Tous les montants sont en
